    ...
    : IDentistCommandService
{
    public async Task<Dentist?> Handle(CreateDentistCommand command)
    {
        ...
        var existingDentistByFullName =
            await dentistRepository.FindByFullNameAsync(command.first_name, command.last_name);
        if (existingDentistByFullName != null)
            throw new InvalidOperationException("A dentist with this full name already exists.");

        var existingDentistByEmail = await dentistRepository.FindByEmailAsync(command.email);
        if (existingDentistByEmail != null)
            throw new InvalidOperationException("A dentist with this email already exists.");

        var existingDentistByPhone = await dentistRepository.FindByPhoneAsync(command.phone);
        if (existingDentistByPhone != null)
            throw new InvalidOperationException("A dentist with this phone already exists.");
    ...
    public async Task<Dentist?> Handle(UpdateDentistCommand command, int id)
    {
        ...
        var existingDentistByFullName =
            await dentistRepository.FindByFullNameAsync(command.first_name, command.last_name);
        if (existingDentistByFullName != null && existingDentistByFullName.id != id)
            throw new InvalidOperationException("This full name already belongs to a dentist.");

        var existingDentistByEmail = await dentistRepository.FindByEmailAsync(command.email);
        if (existingDentistByEmail != null && existingDentistByEmail.id != id)
            throw new InvalidOperationException("This email already belongs to a dentist.");

        var existingDentistByPhone = await dentistRepository.FindByPhoneAsync(command.phone);
        if (existingDentistByPhone != null && existingDentistByPhone.id != id)
            throw new InvalidOperationException("This phone already belongs to a dentist.");
    ...
    public async Task Handle(DeleteDentistCommand command)
    {
        ...
        if (dentist == null) throw new KeyNotFoundException($"Dentist with ID {command.id} not found.");
